...
void rand_bn(PRG &prg,Group *G,BigInt &a){
	unsigned char buf[512];
	prg.random_data(buf,G->order.size());
	a.from_bin(buf,G->order.size());
}

template<typename IO>
class BOT: public OT<BOT<IO>> { public:
	IO* io;
	Group *G = nullptr;
	bool delete_G = true;
	PRG prg;
	BOT(IO* io, PRG &prng, Group * _G = nullptr) {
...
public:
	...
	DOT(IO * io, block * pretable, PRG &prng,int ssp = 40) {
		...
		this->io = io;
		this->ssp = ssp;
		this->base_ot = new BOT<IO>(io,prng);
		this->sspover8 = ssp/8;
		this->l +=ssp;
		this->s = new bool[l];
